...
dataset = pd.DataFrame.from_dict({
    ...
    "gold_label": [('entailment', 'contradiction')[d['label']] for d in dataset]
})
...
valid_dataset = pd.DataFrame.from_dict({
    ...
    "gold_label": [('entailment', 'contradiction')[d['label']] for d in valid_dataset]
})
...
model = WARPPromptedRobertaForSequenceClassification(
                                                     ...
                                                     mask_token_id = 50264,
                                                     token_ids_for_classification_head = [10932, 38908],
...
def validate():
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(0, valid_dataset.shape[0], args.eval_batch_size)):
            ...
            valid_features = tokenizer([valid_dataset.sentence1.iloc[i] + "?" + middle_string + valid_dataset.sentence2.iloc[i] for i in exs], return_tensors='pt', truncation=True, padding=True)
...
for step in tqdm.tqdm(range(250000)):
    ...
    train_features = tokenizer([dataset.sentence1.iloc[i] + "?" + middle_string +  dataset.sentence2.iloc[i] for i in exs], return_tensors='pt', truncation=True, padding=True)
