...
from models.modeling_roberta import (
    WARPPromptedRobertaForMaskedLM,
    WARPPromptedRobertaForSequenceClassification,
)
...
parser.add_argument("--total_train_steps", default=250000, type=int)
parser.add_argument(
    "--seed_token_id_for_prompts_embeddings", default=50264, type=int
)  # used token id for "<mask>"
parser.add_argument("--mask_token_id", default=50264, type=int)
parser.add_argument(
    "--token_ids_for_classification_head",
    action="append",
    dest="token_ids_for_classification_head",
    default=[1342, 12516, 10800],
)  #'ent', 'neutral', 'cont'
...
dataset = pd.DataFrame.from_dict(
    {
        ...
        "gold_label": [
            ("entailment", "neutral", "contradiction")[d["label"]] for d in dataset
        ],
    }
)
...
valid_dataset = pd.DataFrame.from_dict(
    {
        ...
        "gold_label": [
            ("entailment", "neutral", "contradiction")[d["label"]]
...
model = WARPPromptedRobertaForSequenceClassification(
    ...
    seed_token_id_for_prompts_embeddings=args.seed_token_id_for_prompts_embeddings,
    mask_token_id=args.mask_token_id,
    token_ids_for_classification_head=args.token_ids_for_classification_head,
...
optim = torch.optim.Adam(
    list(filter(lambda x: x.requires_grad, model.parameters())), lr=args.learning_rate
)
warmup = int(args.warmup_proportion * args.cycle)
scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
    optim, args.cycle, T_mult=args.tmult
)
...
def validate():
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(0, valid_dataset.shape[0], args.eval_batch_size)):
            ...
            valid_features = tokenizer(
                [
                    valid_dataset.premise.iloc[i]
                    + "?"
                    + middle_string
                    + valid_dataset.hypothesis.iloc[i]
                    for i in exs
                ],
                return_tensors="pt",
                truncation=True,
                padding=True,
                max_length=args.max_seq_length - args.n_prompts,
            )
            valid_features["input_ids"] = torch.cat(
                [
                    torch.full(
                        (valid_features["input_ids"].shape[0], args.n_prompts), 0
                    ),
                    valid_features["input_ids"],
                ],
                1,
            )
            valid_features["input_ids"] = valid_features["input_ids"].to(device)
            valid_features["attention_mask"] = torch.cat(
                [
                    torch.full(
                        (valid_features["input_ids"].shape[0], args.n_prompts), 1
                    ),
                    valid_features["attention_mask"],
                ],
                1,
            )
            valid_features["attention_mask"] = valid_features["attention_mask"].to(
                device
            )

            lbl = (
                torch.Tensor(
                    [label_dict[valid_dataset.gold_label.iloc[i]] for i in exs]
                )
                .type(torch.int64)
                .cuda()
            )
    ...
    acc = total_correct / total_seen * 100.0
    print({"val_acc": acc})
    return acc
...
for step in tqdm.tqdm(range(args.total_train_steps)):
    exs = random.sample(range(0, dataset.shape[0]), args.train_batch_size)
    train_features = tokenizer(
        [
            dataset.premise.iloc[i] + "?" + middle_string + dataset.hypothesis.iloc[i]
            for i in exs
        ],
        return_tensors="pt",
        truncation=True,
    ...
    train_features["input_ids"] = torch.cat(
        [
            torch.full((train_features["input_ids"].shape[0], args.n_prompts), 0),
            train_features["input_ids"],
        ],
        1,
    )
    train_features["input_ids"] = train_features["input_ids"].to(device)
    train_features["attention_mask"] = torch.cat(
        [
            torch.full((train_features["input_ids"].shape[0], args.n_prompts), 1),
    ...
    lbl = (
        torch.Tensor([label_dict[dataset.gold_label.iloc[i]] for i in exs])
        .type(torch.int64)
        .to(device)
    )
    ...
    loss = (
        torch.nn.functional.cross_entropy(bleh, lbl) / args.gradient_accumulation_steps
    )
    loss.backward()
    ...
    if step % 1000 == 0:
        ...
        if acc > best_acc:
            best_acc = acc
            cnt = 0
            best_soft_embedding = copy.deepcopy(
                model.backbone.roberta.embeddings.word_embeddings
            )
        ...
        if args.use_tensorboard:
            writer.add_scalar(
                "avg train loss", loss.item() / args.train_batch_size, step
            )
            writer.add_scalar("acc val", acc, step)

        if cnt >= args.patience:
            best_soft_embedding.save_pretrained_soft_prompts(args.save_prompts_path)
            best_classification_head.save_pretrained_classifier(
                args.save_classifier_path
            )
