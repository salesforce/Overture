...
from models.modeling_bert import WARPPromptedBertForMaskedLM, WARPPromptedBertForSequenceClassification
...
parser.add_argument("--backbone_model", default="bert-large-uncased", type=str)
...
# initialize model
model = WARPPromptedBertForSequenceClassification(
                                                     pretrained_backbone_path = args.backbone_model,                 
                                                     n_prompts = args.n_prompts, 
                                                     seed_token_id_for_prompts_embeddings = 103,
                                                     mask_token_id = 103,
                                                     token_ids_for_classification_head = [4372, 8699, 10800], #'en', 'neutral', 'contra'
...
def validate():
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(0, valid_dataset.shape[0], args.eval_batch_size)):
            ...
            valid_features["token_type_ids"] = torch.cat([torch.full((valid_features["input_ids"].shape[0], args.n_prompts), 0), valid_features['token_type_ids']], 1)
            valid_features["token_type_ids"] = valid_features["token_type_ids"].to(device)
...
for step in tqdm.tqdm(range(250000)):
    ...
    train_features["token_type_ids"] = torch.cat([torch.full((train_features["input_ids"].shape[0], args.n_prompts), 0), train_features['token_type_ids']], 1)
    train_features["token_type_ids"] = train_features["token_type_ids"].to(device)
    ...
    if step % 1000 == 0:
        ...
        if acc > best_acc:
            best_acc = acc
            cnt = 0
            best_soft_embedding = copy.deepcopy(model.backbone.bert.embeddings.word_embeddings)
