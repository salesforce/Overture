...
from template_factory import NLIConverter
...
converter = NLIConverter(mask_token = "<mask>")
...
for step in tqdm.tqdm(range(250000)):
    ...
    train_features = tokenizer([converter.convert_for_mlm(premise = dataset.premise.iloc[i], hypothesis = dataset.hypothesis.iloc[i], relationship = dataset.label.iloc[i]) for i in exs], return_tensors='pt', truncation=True, padding=True)
