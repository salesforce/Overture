...
for step in tqdm.tqdm(range(250000)):
    ...
    train_features = tokenizer([dataset.premise.iloc[i] + "?" + dataset.gold_label.iloc[i] +  dataset.hypothesis.iloc[i] for i in exs], return_tensors='pt', truncation=True, padding=True)
