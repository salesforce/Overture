...
dataset = pd.DataFrame.from_dict({
    "sentence1": [d['sentence1'] for d in dataset],
    "sentence2": [d['sentence2'] for d in dataset],
    "gold_label": [('yes', 'instead')[d['label']] for d in dataset]
})
valid_dataset = load_dataset("glue", "rte", split="validation")
valid_dataset = pd.DataFrame.from_dict({
    "sentence1": [d['sentence1'] for d in valid_dataset],
    "sentence2": [d['sentence2'] for d in valid_dataset],
    "gold_label": [('yes', 'instead')[d['label']] for d in valid_dataset]
})
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(0, valid_dataset.shape[0], args.eval_batch_size)):
            ...
            if exs[-1] >= valid_dataset.shape[0] or len(exs)<2:
                ...
                break
            total_seen += len(exs)
            #valid_tokenized_ip = tokenizer(list(zip(valid_dataset.sentence1.tolist()[exs], valid_dataset.sentence2.tolist()[exs])), return_tensors='pt', truncation=True, padding=True).to(device)

            valid_features = tokenizer([valid_dataset.sentence1.iloc[i] + "?" + valid_dataset.gold_label.iloc[i] + valid_dataset.sentence2.iloc[i] for i in exs], return_tensors='pt', truncation=True, padding=True)
            ...
            # mask inputs
            valid_features['input_ids'] = random_mask_input_ids(valid_features['input_ids'], 50264)
...
for step in tqdm.tqdm(range(250000)):
    ...
    train_features = tokenizer([dataset.sentence1.iloc[i] + "?" + dataset.gold_label.iloc[i] +  dataset.sentence2.iloc[i] for i in exs], return_tensors='pt', truncation=True, padding=True)
    ...
    # mask inputs
    train_features['input_ids'] = random_mask_input_ids(train_features['input_ids'], 50264, args.random_mask_rate)
    ...
    if (step % args.gradient_accumulation_steps) == 0:
        ...
        # print(model.backbone.roberta.embeddings.word_embeddings.soft_prompts)
