...
from models.modeling_xlm_roberta import WARPPromptedXLMRobertaForMaskedLM, WARPPromptedXLMRobertaForSequenceClassification
...
# argument train_language has to be within ['ar', 'bg', 'de', 'en', 'el', 'es', 'fr', 'hi', 'ru', 'sw', 'th', 'tr', 'ur', 'vi', 'zh']
parser.add_argument("--train_language", default="en", type=str)
parser.add_argument("--train_batch_size", default=16, type=int)
parser.add_argument("--eval_language", default="en", type=str)
...
# load data
dataset = load_dataset('xnli', args.train_language, split='train')
...
valid_dataset = load_dataset('xnli', args.eval_language, split='validation')
...
# initialize model
model = WARPPromptedXLMRobertaForSequenceClassification(
                                                     pretrained_backbone_path = args.backbone_model,                 
                                                     n_prompts = args.n_prompts, 
                                                     seed_token_id_for_prompts_embeddings = 250001,
                                                     mask_token_id = 250001,
                                                     token_ids_for_classification_head = [22, 62276, 2304], #'_en', 'neutral', '▁contra'
...
def validate():
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(0, valid_dataset.shape[0], args.eval_batch_size)):
            ...
            valid_features = tokenizer([valid_dataset.premise.iloc[i] + "?" + middle_string + valid_dataset.hypothesis.iloc[i] for i in exs], return_tensors='pt', truncation=True, padding=True, max_length=args.max_seq_length - args.n_prompts)
...
for step in tqdm.tqdm(range(total_steps)):
    ...
    train_features = tokenizer([dataset.premise.iloc[i] + "?" + middle_string +  dataset.hypothesis.iloc[i] for i in exs], return_tensors='pt', truncation=True, padding=True, max_length=args.max_seq_length - args.n_prompts)
    ...
    if step % 1000 == 0 or step == (total_steps-1):
        ...
        if cnt >= args.patience or step == (total_steps-1):
            ...
            raise Exception("running out of patience or training completed!!")
