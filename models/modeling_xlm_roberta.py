...
class WARPPromptedXLMRobertaForMaskedLM(nn.Module):
    def __init__(self, 
        ...
        """
        pretrained_backbone_path: str, path to or name of backbone model, e.g. xlm-roberta-large;                
        ...
        """
        super(WARPPromptedXLMRobertaForMaskedLM, self).__init__()
...
class WARPPromptedXLMRobertaForSequenceClassification(nn.Module):
    def __init__(self, 
                 pretrained_backbone_path,                
                 n_prompts, 
                 seed_token_id_for_prompts_embeddings,
        ...
        n_prompts: int, number of prompts;
        seed_token_id_for_prompts_embeddings: int, use embedding of a specific token to initialize prompts weights, usually use mask token;
        mask_token_id: int, token id for mask token, 250001 for huggingface xlm-roberta model;
        ...
        """
        super(WARPPromptedXLMRobertaForSequenceClassification, self).__init__()
        self.backbone = AutoModelForMaskedLM.from_pretrained(pretrained_backbone_path)
        self.n_prompts = n_prompts
        ...
        # classification head
        weights4lm_head = torch.nn.Parameter(self.backbone.roberta.embeddings.word_embeddings.ori_emb.weight[token_ids_for_classification_head])
        prediction_dim = len(token_ids_for_classification_head)
        self.classification_head = XLMRobertaClassificationHead(self.backbone.lm_head, 
                                                             weights4lm_head, 
                                                             hidden_size, 
                                                             prediction_dim)
...
class WARPPromptedXLMRobertaForQuestionAnswering(nn.Module):
    def __init__(self, 
                 pretrained_backbone_path,                 
                 n_prompts, 
                 seed_token_id_for_prompts_embeddings,
        ...
        seed_token_id_for_prompts_embeddings: int, use embedding of a specific token to initialize prompts weights, usually use mask token.
        """
        super(WARPPromptedXLMRobertaForQuestionAnswering, self).__init__()
    ...
    def forward(self, input_ids, attention_mask, start_positions = None, end_positions = None):
        return self.backbone(input_ids, attention_mask, start_positions = start_positions, end_positions = end_positions)
