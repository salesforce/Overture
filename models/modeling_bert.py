...
class WARPPromptedBertForSequenceClassification(nn.Module):
    def __init__(
        ...
        weights4lm_head = torch.nn.Parameter(
            self.backbone.bert.embeddings.word_embeddings.ori_emb.weight[
                token_ids_for_classification_head
            ]
        )
        prediction_dim = len(token_ids_for_classification_head)
        self.classification_head = BertClassificationHead(
            self.backbone.cls, weights4lm_head, hidden_size, prediction_dim
        )
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        before_classifier = self.backbone.bert(
