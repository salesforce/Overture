...
class PromptedWordEmbeddings(nn.Module):
    def __init__(
        ...
        self.soft_prompts = (
            torch.zeros(n_prompts, hidden_size)
            + original_word_embeddings.weight[seed_token_id_for_prompts_embeddings]
            .clone()
            .detach()
        )
        self.soft_prompts = nn.Parameter(self.soft_prompts, requires_grad=True)
        logger.info(
            f"initialized soft prompts with dimension: {self.soft_prompts.shape}"
        )
    ...
    def load_from_pretrained_soft_prompts(self, pretrained_prompts_path):
        ...
        if pretrained_soft_prompts.shape[0] == self.n_prompts:
            ...
            raise Exception(
                f"pretrained soft prompts dimension: {pretrained_soft_prompts.shape}, but initialized with {self.soft_prompts.shape}"
            )
